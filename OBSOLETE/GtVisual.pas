...
{ dodatkowo wymusza zaznacza zawartoæ kontrolek edycyjnych na wypadek gdyby otrzyma³y Focus wczeniej i w miêdzyczasie zmieni³a siê ich zawartoæ }
procedure SetCtlFocus(aComponent: TWinControl);
begin
...
{ wzraca Tag wskazanego obiektu, funkcja niepotrzebna ale u³atwia zrozumienie i czytelnoæ kodu ;-) }
function  GetSenderTag(Sender: TObject): Integer;
begin
  Result := 0;
  try
...
{ zwraca True jeli wybrano Tak lub Nie }
function  MsgBoxYesNoCanc(aCapt,aText: String;var aYesNo: Boolean): Boolean;
var i: Integer;
begin
...
{ przenosi Menu item do innego menu, wykorzystuje w³asnoæ GroupIndex }
{ TODO: co jeli pozycja o danej nazwie ju¿ istnieje ??? }
...
{ zwraca iloæ zajêtej pamiêci jako string, odpowiednio sformatowany jako kB, MB, GB itp }
function GtFileOrMemSizeToStr(aSize: Integer; aPrec: Integer=-1): String;
begin
